...
function funcaojuros(){
        ...
        document.getElementById("tituloboleto").innerHTML = "Lista de Boletos com juros"
        //calcula os juros
    ...
    document.getElementById("valortotal").innerHTML = "Total Acumulado:" + parseFloat(sum);
}

//funcao para agrupar clientes em uma tabela
function funcaoagruparcliente(){
    //mostra o titulo da tabela 
    document.getElementById("tituloboleto").innerHTML = "Lista de Boletos por Cliente"
    ...
    function funcaobuscar(item, index){
        if (document.getElementById("buscapornome").value === item.NomeCliente){
            ...
            console.log(JSON.stringify(clientes[index]));
            console.log("clientesindex")
        }
    }
    
    console.log("clienteespecifico" + JSON.stringify(clientes));
...
function funcaoagrupardata(){
    //mostra o titulo da tabela 
    document.getElementById("tituloboleto").innerHTML = "Lista de Boletos por Data de Vencimento"
    console.log(JSON.stringify(clientes));
    //coloca na const cliente especifico todos os boletos de determinado cliente 
    clientes.forEach(funcaobuscar);
    function funcaobuscar(item, index){
        if (document.getElementById("buscarpordatavencimento").value === item.DataVencimento){
            clienteespecifico.push(clientes[index])
            console.log(JSON.stringify(clientes[index]));
            console.log("clientesindex")
        }
